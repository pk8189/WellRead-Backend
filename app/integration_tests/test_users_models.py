...
def test_create_user(client):
    ...
    assert (
        client.post(
            "/api/token", data={"username": "pmkelly4444@gmail.com", "password": "1"}
        ).json()["detail"]
    ...
    assert (
        client.post(
            "/api/token", data={"username": "invalid", "password": "string"}
        ).json()["detail"]
        == "Incorrect username or password"
    )
...
def test_unfollow_user(client):
    ...
    api_util.create_user2_and_authenticate()
    client.put(f"/api/user/relationship/{user1_id}/unfollow/")
    assert not len(client.get("/api/user/").json()["following"])

    api_util.create_user_and_authenticate()
    assert not len(client.get("/api/user/").json()["followers"])
